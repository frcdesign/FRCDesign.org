...
!!! Note 
    The Origin Cube also has additional functionality for robot and mechanism assemblies that will be discussed in Stage 1C. You can also check out these practices on the [assembly best practices page](../../../best-practices/assembly-setup.md "") The Origin Cube feature must be **the first feature in all part studios** from here on out in order for the functions to be available.
